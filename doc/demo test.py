...
code = 'AQDHYUhXt3_CEHxyCItRXIUBsxIZZm2gAuJOHOnGlGdLAh9cUSXPERHEbGgQNE3J1nBOHCFNLQ1YAG6uGx7h8Gp1nTS8ecA3BeeyAkQizz7djsypzShgAxkg3Kb87wstEM9gAOdOk4z8dX-ZgiEa2D6jsTbh-k4ezrRDAV_wQDs8bwj8Hvicgczo6ahcBuESsK0Wn_owrHY44jbUdnbDGVEWl5FOCJgLsmMyWggsc10dpUbbHfJcFnJjTGhr7dm7spDJOQBPPl5Yu1icB5pguYwdfpZX6P_F1Fq_P8FlKHGdbQ'
...
def get_tile_metadata(tiles, access_token):
    # Get the first tile from the list
    tile = tiles[1]
    ...
    with open('tile_metadata.json', 'w') as f:
        f.write(data)
    print('Tile metadata saved successfully!')
    
    return 'tile_metadata.json'  # Return the filename for use in other functions
...
def main():
    ...
    metadata_file = get_tile_metadata(tiles, access_token)
    
    save_dir = create_save_directory(os.path.join(r"C:\svi\mapillary-api\data", f"svi_{city}"))
    
    download_images(metadata_file, save_dir, access_token, city)
